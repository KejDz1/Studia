...
int main()
{
    ...
    for (int i = 1; i <= Max; i++)
    {
        ...
        const int DataSize = pow(10, i);
        for (int j = 0; j < DataSize; j++)
        {
            int RandomData = rand() % 100 + 1;
            lista->AddOnFront(RandomData);
        }

        clock_t tEnd = clock();
        double time = (tEnd - tStart)/(double)CLOCKS_PER_SEC;
        lista->DisplayLinkedList(10);
        cout << "The time of this operation: " << time << endl;
        double AddingTime = time;
        const int SearchSize = pow(10, 2);

        tStart = clock();
        for (int k = 0; k < SearchSize; k++)
        {
            int RandomKey = rand() % 100 + 1;
            lista->EraseByKey(RandomKey);
        ...
        lista->DisplayLinkedList(10);
        cout << "The time of this operation: " << time << endl;
        cout << "The time of adding operation is: " << AddingTime << endl;        
    }
